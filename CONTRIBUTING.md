...
**Squash-merge only.** Every PR lands as exactly one commit on `main`, with the
title taken from the PR title and the body from the PR description (GitHub does
this automatically). Merge-commit and rebase-merge are disabled at the repo
level, so there is nothing to choose — open a clean PR and squash.

Why this policy, given we value reproducibility and transparency:

- **Reproducible `main`.** Every commit is one complete, reviewed, CI-green
  change, so each commit builds and passes tests and `git bisect` never lands on
  a half-finished intermediate state.
- **Transparency is preserved, not lost.** The per-subtask granularity from §2.4
  — individual branch commits, the full diff, and the review discussion — lives
  on the PR forever and is linked from the squash commit's `(#NN)` suffix. `main`
  stays a clean ledger; the fine-grained history lives on the PR where
  contributors actually read it.
- **Low friction for many contributors.** Squash absorbs varied commit hygiene
  (`wip`, `fix typo`) into one clean commit and avoids the rebase/force-push
  hazards that trip up newcomers.

Because the squash commit message comes from the PR, **write PR titles as
Conventional Commits** (`fix:`, `feat:`, `chore:`, `docs:`) matching your branch
prefix (§2.3), and treat the PR description as the commit body you want on
`main`.
...
- **Squash-merge only, linear history:** merge-commit and rebase-merge are
  disabled and `main` requires linear history, so every PR becomes exactly one
  commit on `main` (see §3).
