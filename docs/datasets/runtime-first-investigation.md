...
- **FRESCO (#26)** — Purdue/TACC. Job accounting (submit/start/end, requested resources, user,
  ...
  `datadepot.rcac.purdue.edu/sbagchi/fresco/`. **Gotcha: untrusted TLS cert chain** — strict
  fetchers fail; use Globus / browser / `curl -k`.
...
2. **FRESCO datadepot** has an untrusted TLS cert chain — strict HTTPS fetchers fail; use
   Globus / browser / `curl -k`. Flag in the downloader.
