...
1. **Bin edges are derived from the training rows** — the modal values of `requested_seconds` that each hold at least `min_cluster_fraction` of the window become the boundaries (falling back to quantiles when the distribution has no modes), so each system contributes its own partition limits instead of one site's numbers being hardcoded. Every detected cluster becomes an edge, so it sits at the top of its own bin, and one open bin holds whatever exceeds the largest cluster — `n_wallclock_bins` therefore bounds the clusters detected, and k of them yield up to k+1 bins. On the Kestrel benchmark window this derives `1, 2, 4, 24, 48` hours. `wallclock_bin_edges_hours` pins them explicitly when the layout is known.
...
4. **Recency weighting** (`time_decay_rate`, default `0.0` — off) weights each training row by `exp(-rate * days_old)`; `0.05` is roughly a two-week half-life. It defaults off so that a comparison against another model isolates the routing rather than bundling it with reweighting.
